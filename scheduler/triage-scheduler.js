...
function buildPrompt(context) {
  return [
    ...
    "Call SecurityOps.ClaimAlert first. If status is acquired, preserve its claimToken exactly and include that same claimToken in every request to GetAlertContext, EnrichAlert, MatchKnowledge, EvaluatePolicy, RecordTriageResult, CreateManualTicket, QueueFeishuNotification, and FinalizeTriage.",
    ...
    "Return only the structured JSON required by the provided schema. polled and ingested must both be 0. processed must equal the length of traceIds and terminalStates, which use matching order.",
    "Mode: " + context.mode,
  ].filter(Boolean).join("\n");
}
