...
function buildPrompt(context) {
  return [
    ...
    "Use these exact request fields: GetAlertContext {eventId,claimToken}; EnrichAlert {traceId,claimToken}; MatchKnowledge {traceId,claimToken}; EvaluatePolicy {traceId,claimToken}; RecordTriageResult {traceId,narrative,claimToken}; CreateManualTicket {traceId,resultId,claimToken}; QueueFeishuNotification {traceId,ticketId,claimToken}; FinalizeTriage {traceId,claimToken}.",
    ...
    "EnrichAlert returns the authoritative context for your narrative. MatchKnowledge and EvaluatePolicy independently rebuild that context from the claimed trace, so never send or invent classification, context, evidence, or knowledge identifiers in those requests.",
    "The authoritative decision, action and executable-rule evaluation come only from EvaluatePolicy. RecordTriageResult accepts only your evidence-grounded narrative; SecurityOps binds it to the stored policy.",
