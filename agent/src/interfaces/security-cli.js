...
import { SecurityTriageAgent } from "../application/pipelines/security-triage-pipeline.js";
...
import { requiredConfig } from "../config/env.js";
...
try {
  ...
  if (recoverOutbox) {
    ...
    // 留痕层：终态审计记录（结论 + 证据引用 + 原始入参 + 模型来源 + prompt 版本）。
    try {
      audit.append({
