...
该目录是独立的 OctoBus service package。它使用最小权限只读账号查询 Wazuh Indexer 的 `wazuh-alerts-*` 索引，并通过 `ListAlerts` unary 方法向获授权的 Agent 提供告警。部署可用 `required_rule_group` 限定安全运营入口；本仓库固定为 `triage_input`，避免把 Wazuh 自身运维事件误送入研判队列。Wazuh/OpenSearch 文档 ID 按不透明值处理：可直接作为内部键的 ID 保持不变，其他合法非空 ID 转为稳定哈希，原始文档 ID 和索引名保留在告警 JSON 的 `_triage_source` 中用于追溯。
